...
local function Ripple(obj)
    spawn(function()
        ...
        local posX = (mx - Ripple.AbsolutePosition.X) / obj.AbsoluteSize.X
        local posY = (my - Ripple.AbsolutePosition.Y) / obj.AbsoluteSize.Y
...
function Library.new(Library, name)
    ...
    function window.Tab(window, name, icon)
        ...
        function tab.Section(tab, name, TabVal)
            ...
            local colorPickerSingleton = nil
            function section.Colorpicker(section, text, flag, defaultColor, callback)
                ...
                local function createPicker()
                    if colorPickerSingleton and colorPickerSingleton.Parent then
                        colorPickerSingleton.Enabled = true
                        return
                    end
                    ...
                    local function updatePickerUI()
                        ...
                        local newDisplay = mainFrame:FindFirstChild("NewDisplayFrame")
                    ...
                    updatePickerUI()
                    colorPickerSingleton = pickerGui
                end

                ColorBtn.MouseButton1Click:Connect(function()
                    if not colorPickerSingleton then
                        createPicker()
                    else
                        colorPickerSingleton.Enabled = true
                    end
            ...
            function section.Keybind(section, text, default, callback)
                ...
                local banned = {
                    Return = true, Space = true, Tab = true, Backquote = true, CapsLock = true, Escape = true, Unknown = true
                }
                local shortNames = {
            ...
            function section.Slider(section, text, flag, default, min, max, precise, callback)
                ...
                MinSlider.MouseButton1Down:Connect(function()
                    changeValue(-step)
                    minHold = task.spawn(function()
                        while true do
                            task.wait(0.1)
                            if not minHold then break end
                            changeValue(-step)
                        end
                    end)
                end)
                MinSlider.MouseButton1Up:Connect(function()
                    if minHold then task.cancel(minHold); minHold = nil end
                end)

                AddSlider.MouseButton1Down:Connect(function()
                    changeValue(step)
                    maxHold = task.spawn(function()
                        while true do
                            task.wait(0.1)
                            if not maxHold then break end
                            changeValue(step)
                        end
                    end)
                end)
                AddSlider.MouseButton1Up:Connect(function()
                    if maxHold then task.cancel(maxHold); maxHold = nil end
                end)
                ...
                SliderValue:GetPropertyChangedSignal("Text"):Connect(function()
                    ...
                    if num then
                        ...
                        if precise then num = tonumber(string.format("%.1f", num)) end
                        funcs:SetValue(num)
                    end
                end)
            ...
            function section.Dropdown(section, text, flag, options, default, callback)
                ...
                local searchDropdown = function(searchText)
                    if searchText == "" then
                        setAllVisible()
                        return
                    end
                    for _, opt in ipairs(DropdownModule:GetChildren()) do
                        if opt:IsA("TextButton") and opt.Name:match("Option_") then
                            opt.Visible = opt.Text:lower():match(searchText:lower()) ~= nil
                        end
                    end
                end
